...
func uploads(router *httprouter.Router, fsys MkdirFS) {
	router.POST("/_apis/pipelines/workflows/:runId/artifacts", WithLogging(BearerAuth(func(w http.ResponseWriter, req *http.Request, params httprouter.Params) {
	...
	router.PUT("/upload/:runId", WithLogging(BearerAuth(func(w http.ResponseWriter, req *http.Request, params httprouter.Params) {
	...
	router.PATCH("/_apis/pipelines/workflows/:runId/artifacts", WithLogging(BearerAuth(func(w http.ResponseWriter, req *http.Request, params httprouter.Params) {
...
func downloads(router *httprouter.Router, fsys fs.FS) {
	router.GET("/_apis/pipelines/workflows/:runId/artifacts", WithLogging(BearerAuth(func(w http.ResponseWriter, req *http.Request, params httprouter.Params) {
		runID := params.ByName("runId")
	...
	router.GET("/download/:container", WithLogging(BearerAuth(func(w http.ResponseWriter, req *http.Request, params httprouter.Params) {
	...
	router.GET("/artifact/*path", WithLogging(BearerAuth(func(w http.ResponseWriter, req *http.Request, params httprouter.Params) {
...
func Serve(ctx context.Context) context.CancelFunc {
	...
	if CLI.Server.ArtifactPath == "" {
		panic("artifactPath is empty")
	}

	router := httprouter.New()

	fmt.Printf("Artifacts base path: %s", CLI.Server.ArtifactPath)
	fs := os.DirFS(CLI.Server.ArtifactPath)
	uploads(router, MkdirFsImpl{CLI.Server.ArtifactPath, fs})
	downloads(router, fs)

	server := &http.Server{
		Addr:              CLI.Server.Bind,
		ReadHeaderTimeout: 2 * time.Second,
		Handler:           router,
	}

	// run server
	log.WithField("addr", CLI.Server.Bind).Info("starting server")
